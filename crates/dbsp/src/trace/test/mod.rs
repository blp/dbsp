...
use crate::{
    ...
    dynamic::{pair::DynPair, DowncastTrait, DynData, DynWeightedPairs, Erase, LeanVec, Unit},
    trace::{
        cursor::CursorPair,
        ord::{
            file::{
...
) -> Box<DynWeightedPairs<DynPair<DynI32, Unit>, DynZWeight>> {
...
fn test_zset_spine<B: ZSet<Key = DynI32>>(
    ...
    let mut ref_trace: TestBatch<DynI32, Unit /* <()> */, (), DynZWeight> =
    ...
    for (tuples, bound) in batches.into_iter() {
        ...
        let ref_batch: TestBatch<DynData, Unit, (), DynZWeight> =
...
fn test_zset_trace_spine<B: ZBatch<Key = DynI32, Val = Unit, Time = u32>>(
    factories: &B::Factories,
    batches: Vec<(Vec<Tup2<i32, ZWeight>>, i32)>,
    seed: u64,
) {
    let mut trace1: Spine<B> = Spine::new(factories, "");
    let mut trace2: Spine<B> = Spine::new(factories, "");
    let mut ref_trace: TestBatch<DynI32, Unit /* <()> */, u32, DynZWeight> =
        TestBatch::new(&TestBatchFactories::new(), "");
