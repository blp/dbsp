...
use std::{sync::Mutex, thread::park};
use textwrap::indent;
use uuid::Uuid;
    ...
    B: Batch,
{
    fn add_batches(&mut self, batches: impl IntoIterator<Item = Arc<B>>) {
        for batch in batches {
            self.slots[size_to_level(batch.len())]
                .loose_batches
                .push(batch);
        }
    }
    ...
    fn merge_complete(&mut self, level: usize, new_batch: Arc<B>) {
        self.slots[level].merging_batches = None;
        self.slots[size_to_level(new_batch.len())]
            .loose_batches
            .push(new_batch);
    }
}
...
{
    fn new() -> Self {
        let quiescent = Arc::new(Condvar::new());
        ...
        let join_handle = Some(
            std::thread::Builder::new()
                .name("async-merger".to_string())
                .spawn({
                    let state = Arc::clone(&state);
                    let quiescent = Arc::clone(&quiescent);
                    move || Self::run(state, quiescent)
                })
                .unwrap(),
        );
        Self {
    ...
    fn run(state: Arc<Mutex<SharedState<B>>>, quiescent: Arc<Condvar>) {
        let mut mergers: [Option<(B::Merger, [Arc<B>; 2])>; MAX_LEVELS] =
            std::array::from_fn(|_| None);
        while !state.lock().unwrap().request_exit {
            ...
            for (level, m) in mergers.iter_mut().enumerate() {
                if let Some((merger, [a, b])) = m.as_mut() {
                    ...
                    if fuel >= 0 {
                        let new_batch = Arc::new(m.take().unwrap().0.done());
            ...
            // If no merges are happening, none will be started until more batches are submitted.
            if mergers.iter().all(|m| m.is_none()) {
