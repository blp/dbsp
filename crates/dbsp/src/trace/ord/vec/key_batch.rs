use crate::{
    dynamic::{
        DataTrait, DynDataTyped, DynPair, DynVec, DynWeightedPairs, Erase, Factory, LeanVec, Unit,
...
impl<K, T, R> BatchReaderFactories<K, Unit, T, R> for VecKeyBatchFactories<K, T, R>
where
    K: DataTrait + ?Sized,
    T: Timestamp,
    R: WeightTrait + ?Sized,
...
impl<K, R, T> BatchFactories<K, Unit, T, R> for VecKeyBatchFactories<K, T, R>
where
    K: DataTrait + ?Sized,
    T: Timestamp,
    R: WeightTrait + ?Sized,
...
impl<K, T, R, O> Merger<K, Unit, T, R, VecKeyBatch<K, T, R, O>> for OrdKeyMerger<K, T, R, O>
where
    K: DataTrait + ?Sized,
    T: Timestamp,
    R: WeightTrait + ?Sized,
...
impl<'s, K, T, R, O> Cursor<K, Unit, T, R> for OrdKeyCursor<'s, K, T, R, O>
where
    K: DataTrait + ?Sized,
    T: Timestamp,
    R: WeightTrait + ?Sized,
    ...
    O: OrdOffset,
{
    ...
    #[inline]
    fn push(&mut self, kr: &mut WeightedItem<K, Unit, R>) {
