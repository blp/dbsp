...
type RawValBatch<K, V, T, R> = Reader<(K, (), (V, Vec<(T, R)>, ()))>;

type RawKeyCursor<'s, K, V, T, R> =
    FileCursor<'s, K, (), (V, Vec<(T, R)>, ()), (K, (), (V, Vec<(T, R)>, ()))>;

type RawValCursor<'s, K, V, T, R> =
    FileCursor<'s, V, Vec<(T, R)>, (), (K, (), (V, Vec<(T, R)>, ()))>;
...
fn read_filtered<'a, K, A, N, T>(
    cursor: &mut FileCursor<'a, K, A, N, T>,
...
where
    ...
    R: DBWeight,
{
    fn copy_values_if<'s, W>(
        output: &mut Writer2<W, K, (), V, Vec<(T, R)>>,
        key: &K,
        key_cursor: &mut RawKeyCursor<'s, K, V, T, R>,
    ...
    fn copy_value<'s, W>(
        output: &mut Writer2<W, K, (), V, Vec<(T, R)>>,
        cursor: &mut RawValCursor<'s, K, V, T, R>,
        value: &V,
    ) where
        W: Write + Seek,
    {
    ...
    fn merge_values<'s, W>(
        output: &mut Writer2<W, K, (), V, Vec<(T, R)>>,
        cursor1: &mut RawValCursor<'s, K, V, T, R>,
        cursor2: &mut RawValCursor<'s, K, V, T, R>,
        value_filter: &Option<Filter<V>>,
    ) -> bool
    where
        W: Write + Seek,
    ...
    R: DBWeight,
{
    ...
    fn done(mut self) -> FileValBatch<K, V, T, R> {
        FileValBatch {
            file: self.result.take().unwrap_or(Reader::empty().unwrap()),
            lower_bound: 0,
            lower: self.lower,
            upper: self.upper,
        }
...
{
    time: T,
    writer: Writer2<File, K, (), V, Vec<(T, R)>>,
    cur_key: Option<K>,
}
