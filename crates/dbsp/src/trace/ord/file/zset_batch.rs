use crate::{
    ...
    dynamic::{
        DataTrait, DynPair, DynVec, DynWeightedPairs, Erase, Factory, LeanVec, Unit, WeightTrait,
    ...
    R: WeightTrait + ?Sized,
{
    ...
    item_factory: &'static dyn Factory<DynPair<K, Unit>>,
    weighted_item_factory: &'static dyn Factory<WeightedItem<K, Unit, R>>,
    weighted_items_factory: &'static dyn Factory<DynWeightedPairs<DynPair<K, Unit>, R>>,
}
...
impl<K, R> BatchReaderFactories<K, Unit, (), R> for FileZSetFactories<K, R>
where
    K: DataTrait + ?Sized,
    R: WeightTrait + ?Sized,
{
    ...
    fn val_factory(&self) -> &'static dyn Factory<Unit> {
        WithFactory::<()>::FACTORY
    }
...
impl<K, R> BatchFactories<K, Unit, (), R> for FileZSetFactories<K, R>
where
    K: DataTrait + ?Sized,
    R: WeightTrait + ?Sized,
{
    ...
    fn item_factory(&self) -> &'static dyn Factory<DynPair<K, Unit>> {
        self.item_factory
    }

    fn weighted_item_factory(&self) -> &'static dyn Factory<WeightedItem<K, Unit, R>> {
        self.weighted_item_factory
    }

    fn weighted_items_factory(
        &self,
    ) -> &'static dyn Factory<DynWeightedPairs<DynPair<K, Unit>, R>> {
...
where
    K: DataTrait + ?Sized,
    R: WeightTrait + ?Sized,
    Other: BatchReader<Key = K, Val = Unit, R = R, Time = ()>,
{
...
impl<K, R> Merger<K, Unit, (), R, FileZSet<K, R>> for FileZSetMerger<K, R>
where
    Self: SizeOf,
    K: DataTrait + ?Sized,
    R: WeightTrait + ?Sized,
{
    ...
    fn work(
        ...
        _value_filter: &Option<Filter<Unit>>,
        fuel: &mut isize,
    ) {
...
impl<'s, K, R> Cursor<K, Unit, (), R> for FileZSetCursor<'s, K, R>
where
    K: DataTrait + ?Sized,
    R: WeightTrait + ?Sized,
{
    ...
    fn seek_val(&mut self, _val: &Unit) {}

    fn seek_val_with(&mut self, predicate: &dyn Fn(&Unit) -> bool) {
        if !predicate(&()) {
            self.valid = false;
        }
    }
    ...
    fn seek_val_reverse(&mut self, _val: &Unit) {}

    fn seek_val_with_reverse(&mut self, predicate: &dyn Fn(&Unit) -> bool) {
        if !predicate(&()) {
            self.valid = false;
        }
    }
    ...
    fn map_values(&mut self, logic: &mut dyn FnMut(&Unit, &R)) {
        if self.val_valid() {
            logic(&(), self.cursor.current_diff())
        }
    }
    ...
    R: WeightTrait + ?Sized,
{
    ...
    #[inline]
    fn push(&mut self, item: &mut WeightedItem<K, Unit, R>) {
    ...
    fn push_refs(&mut self, key: &K, _val: &Unit, weight: &R) {
        self.builder.push_refs((key, weight))
    }

    fn push_vals(&mut self, key: &mut K, _val: &mut Unit, weight: &mut R) {
