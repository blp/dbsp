...
use crate::{
    ...
    dynamic::{DataTrait, DynPair, DynVec, Erase, Factory, LeanVec, Unit, WithFactory},
    trace::{
    ...
    VCF: Fn(&O, &O) -> Ordering + 'static,
{
    ...
    fn step_key_reverse_n(&mut self, cursor: &mut dyn ZCursor<I, Unit, ()>, mut steps: usize) {
    ...
    fn compute_retractions(
        &mut self,
        input_delta: &mut dyn ZCursor<I, Unit, ()>,
        output_trace: &mut dyn ZCursor<DynPair<I, O>, Unit, ()>,
    ) {
    ...
    fn compute_updates<CB>(
        &mut self,
        input_cursor: &mut dyn ZCursor<I, Unit, ()>,
        mut output_cb: CB,
    ) where
    ...
    VCF: Fn(&O, &O) -> Ordering + 'static,
{
    ...
    fn transform(
        &mut self,
        input_delta: &mut dyn ZCursor<I, Unit, ()>,
        input_trace: &mut dyn ZCursor<I, Unit, ()>,
        output_trace: &mut dyn ZCursor<DynPair<I, O>, Unit, ()>,
...
fn step_key_skip_zeros<I>(cursor: &mut dyn ZCursor<I, Unit, ()>)
where
    I: DataTrait + ?Sized,
{
    cursor.step_key();
...
fn step_key_reverse_skip_zeros<I>(cursor: &mut dyn ZCursor<I, Unit, ()>)
where
    I: DataTrait + ?Sized,
{
...
fn skip_zeros<I>(cursor: &mut dyn ZCursor<I, Unit, ()>)
where
    I: DataTrait + ?Sized,
{
...
fn skip_zeros_reverse<I>(cursor: &mut dyn ZCursor<I, Unit, ()>)
where
    I: DataTrait + ?Sized,
{
    while cursor.key_valid() && cursor.weight().is_zero() {
