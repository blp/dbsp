...
use crate::{
    ...
    circuit_cache_key,
    dynamic::{ClonableTrait, DataTrait, DynPair, Unit},
    ...
    C: Circuit,
{
    ...
    where
        K: DataTrait + ?Sized,
        V: DataTrait + ?Sized,
        CI: BatchReader<Key = DynPair<K, V>, Val = Unit, Time = ()>,
    {
    ...
    where
        CI: BatchReader<Key = DynPair<CO::Key, CO::Val>, Val = Unit, Time = (), R = CO::R>,
        CO: Batch<Time = ()>,
    {
        self.circuit()
...
where
    CO: Batch<Time = ()>,
    CI: BatchReader<Val = Unit, Time = (), R = CO::R>,
