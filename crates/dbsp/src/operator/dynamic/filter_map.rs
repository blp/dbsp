...
use crate::{
    ...
    dynamic::{ClonableTrait, DataTrait, DynPair, Unit, WeightTrait},
    trace::{
...
impl<C: Circuit, B: DynFilterMap> Stream<C, B> {
    ...
    pub fn dyn_map<K: DataTrait + ?Sized>(
        ...
        map_func: Box<dyn Fn(B::DynItemRef<'_>, &mut DynPair<K, Unit>)>,
    ...
    pub fn dyn_flat_map<K: DataTrait + ?Sized>(
        &self,
        output_factories: &OrdWSetFactories<K, B::R>,
        func: Box<dyn FnMut(B::DynItemRef<'_>, &mut dyn FnMut(&mut K, &mut Unit))>,
    ) -> Stream<C, OrdWSet<K, B::R>> {
