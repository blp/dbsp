...
use crate::{
    algebra::Semigroup,
    dynamic::{DataTrait, DowncastTrait, DynOpt, Erase, Factory, Unit, WeightTrait, WithFactory},
    trace::Cursor,
    DBData, DBWeight,
};
...
pub trait Aggregator<K, T, R>: Clone + 'static {
    ...
    fn aggregate<KTrait, RTrait>(
        &self,
        cursor: &mut dyn Cursor<KTrait, Unit, T, RTrait>,
    ...
    R: WeightTrait + ?Sized,
{
    ...
    fn aggregate(
        &self,
        cursor: &mut dyn Cursor<K, Unit, T, R>,
