...
use crate::{
    ...
    dynamic::{
        DataTrait, DynOpt, DynPair, DynPairs, DynSet, Erase, Factory, LeanVec, Unit, Weight,
...
impl<T, R, O> DynAggregator<Unit, T, R> for WeightedCount<R, O>
where
    T: Timestamp,
    R: WeightTrait + ?Sized,
    O: DataTrait + ?Sized,
{
    ...
    fn aggregate(&self, cursor: &mut dyn Cursor<Unit, Unit, T, R>, aggregate: &mut DynOpt<R>) {
    ...
    fn aggregate_and_finalize(
        &self,
        cursor: &mut dyn Cursor<Unit, Unit, T, R>,
    ...
    Z: Clone + 'static,
{
    ...
    where
        Z: IndexedZSet,
        O: Batch<Key = Z::Key, Val = Unit, Time = ()>,
    {
