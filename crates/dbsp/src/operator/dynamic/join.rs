...
impl<I1> Stream<RootCircuit, I1> {
        ...
        Z: ZSet,
    {
        ...
        left.dyn_integrate_trace(stored_self_factories)
            .delay_trace()
            ...
            .plus(&left.dyn_stream_join_inner(
                output_factories,
                &right.dyn_integrate_trace(stored_other_factories),
                join_func,
                Location::caller(),
            ))
    }
...
mod test {
    use crate::{
        ...
        utils::Tup2,
        zset, Circuit, RootCircuit, Runtime, Stream, Timestamp, FileZSet,
    };
            ...
            let result = circuit.iterate(|child| {
                ...
                Ok((move || {
                    ...
                    *counter += 1;
                    ...
                    // FIXME: make sure the `export` API works on typed streams correctly,
                    // so that the `inner`/`typed` calls below are not needed.
                result.integrate_trace().inner().export().typed::<Spine<FileZSet<Label>>>()))
            }).unwrap();
