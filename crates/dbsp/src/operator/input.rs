use crate::{
    ...
    dynamic::{Bool, DowncastTrait, DynData, DynPair, Erase, LeanVec, Unit},
    operator::dynamic::{
        input::{
...
pub struct ZSetHandle<K> {
    handle: CollectionHandle<DynPair<DynData, Unit>, DynZWeight>,
    phantom: PhantomData<fn(&K)>,
}

impl<K> Deref for ZSetHandle<K> {
    type Target = CollectionHandle<DynPair<DynData, Unit>, DynZWeight>;
...
where
    K: DBData,
{
    fn new(handle: CollectionHandle<DynPair<DynData, Unit>, DynZWeight>) -> Self {
        Self {
            handle,
            phantom: PhantomData,
        }
...
pub struct SetHandle<K> {
    handle: UpsertHandle<DynData, Bool>,
    phantom: PhantomData<fn(&K)>,
}
...
where
    K: DBData,
{
    fn new(handle: UpsertHandle<DynData, Bool>) -> Self {
        Self {
            handle,
            phantom: PhantomData,
        }
