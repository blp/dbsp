use crate::{
    dynamic::{DowncastTrait, DynData, Erase, Unit},
    operator::dynamic::{
...
fn mk_trace_join_funcs<I1, I2, Z, F>(
    join: F,
) -> TraceJoinFuncs<I1::DynK, I1::DynV, I2::DynV, Z::DynK, Unit>
where
    I1: IndexedZSet,
    ...
    F: Fn(&I1::Key, &I1::Val, &I2::Val) -> Z::Key + Clone + 'static,
{
    ...
    let mut val1: Box<Unit> = Box::new(()).erase_box();

    let mut key2: Box<Z::DynK> = Box::<Z::Key>::default().erase_box();
    let mut val2: Box<Unit> = Box::new(()).erase_box();
