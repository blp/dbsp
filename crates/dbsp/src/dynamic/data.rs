...
use crate::{
    ...
    dynamic::{
        storage::SerializeDyn, ArchiveTrait, AsAny, Clonable, ClonableTrait, Comparable,
...
/// An object-safe trait that defines the minimal set of operations that must be supported by
/// any value in DBSP.
///
/// This trait is object safe and can be invoked via dynamic dispatch.
pub trait Data:
    Comparable + Clonable + SerializeDyn + DeserializableDyn + Send + Debug + AsAny + SizeOf
{
...
pub type Unit = DynDataTyped<()>;

/// A trait object backed by `bool`.
pub type Bool = DynDataTyped<bool>;
