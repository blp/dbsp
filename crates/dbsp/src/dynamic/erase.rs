use crate::{dynamic::storage::ArchiveTrait, DBData};

/// Type erasure operations.
///
/// Given a trait object type `Trait` such that `Self: Trait` (this constraint cannot be
/// captured explicitly in the Rust type system, hence the need for this trait), converts
/// references to self into references to `Trait`.
